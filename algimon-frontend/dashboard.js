...
async function loadTimeSlotsConfig() {
    if (_timeSlotsConfig) return _timeSlotsConfig;
    try {
        const res  = await fetch(`${API_BASE_URL}/time-slots`, { headers: authHeaders() });
        const data = await res.json();
        if (data.success) _timeSlotsConfig = data.data;
    } catch { /* silent — calendar will work without it */ }
    return _timeSlotsConfig;
}

function isDayActive(jsDate) {
    if (!_timeSlotsConfig) return true; // assume active if not loaded
    const dayName = _dayNames[jsDate.getDay()];
    const cfg = _timeSlotsConfig[dayName];
...
function renderCalendar(target) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (thisDate < minDate) {
            ...
            dayDiv.title = 'This day is currently not available for booking';
            dayDiv.onclick = function () {
                const hintEl = document.getElementById(`cal-hint-${target}`);
                if (hintEl) {
                    const prev = hintEl.innerHTML;
                    hintEl.innerHTML = `<i class="fa-solid fa-ban" style="color:#b91c1c"></i> <span style="color:#b91c1c">This day is not available for booking.</span>`;
